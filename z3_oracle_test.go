...
func TestRandomNonlinearIntegerSystemsAgreeWithPinnedZ3(t *testing.T) {
	z3 := os.Getenv("GOSMT_Z3")
	if z3 == "" {
		t.Skip("set GOSMT_Z3 to the pinned Z3 4.16.0 binary")
	}
...
func TestSMTLibFloatingPointAddAgreeWithPinnedZ3(t *testing.T) {
	z3 := os.Getenv("GOSMT_Z3")
	if z3 == "" {
		t.Skip("set GOSMT_Z3 to the pinned Z3 4.16.0 binary")
	}
	...
	random := rand.New(rand.NewSource(0x46504144))
	for example := 0; example < 160; example++ {
		leftPattern, rightPattern := random.Uint32(), random.Uint32()
		switch example % 32 {
		case 0:
			leftPattern, rightPattern = 0x3f800000, 0x33800000
		case 1:
			leftPattern, rightPattern = 0x3f800000, 0xbf800000
		case 2:
			leftPattern, rightPattern = 0x7f7fffff, 0x7f7fffff
		case 3:
			leftPattern, rightPattern = 0x007fffff, 0x00000001
		case 4:
			leftPattern, rightPattern = 0x7f800000, 0xff800000
		case 5:
			leftPattern, rightPattern = 0x7fc12345, 0x3f800000
		}
		...
		expression := fmt.Sprintf(
			"(fp.add %s ((_ to_fp 8 24) #x%08x) ((_ to_fp 8 24) #x%08x))",
			mode.name, leftPattern, rightPattern,
		)
		assertion := "(fp.isNaN " + expression + ")"
		if !smt.FloatingPointIsNaN(sum) {
			bits, _ := smt.FloatingPointBits(sum).Uint64()
			assertion = fmt.Sprintf(
				"(= (fp.to_ieee_bv %s) #x%08x)", expression, uint32(bits),
			)
		}
		if example%16 >= 8 {
			assertion = "(not " + assertion + ")"
		...
		if got, want := fmt.Sprint(ours), "["+strings.TrimSpace(string(output))+"]"; got != want {
			t.Fatalf(
				"example %d (%s, %#08x + %#08x): gosmt=%s z3=%s\n%s",
				example, mode.name, leftPattern, rightPattern, got, want, script,
			)
		}
	}
}

func TestSMTLibSymbolicFloatingPointAddAgreeWithPinnedZ3(t *testing.T) {
	z3 := os.Getenv("GOSMT_Z3")
	if z3 == "" {
		t.Skip("set GOSMT_Z3 to the pinned Z3 4.16.0 binary")
	}
	...
	random := rand.New(rand.NewSource(0x46505341))
	for example := 0; example < 64; example++ {
		leftPattern, rightPattern := random.Uint32(), random.Uint32()
		if leftPattern&0x7f800000 == 0x7f800000 &&
			leftPattern&0x007fffff != 0 {
		...
		expected, _ := smt.FloatingPointBits(sum).Uint64()
		assertion := fmt.Sprintf(
			"(= (fp.to_ieee_bv (fp.add %s left right)) #x%08x)",
			mode.name, uint32(expected),
		)
		if example%8 >= 4 {
			assertion = "(not " + assertion + ")"
		}
		...
		if got, want := fmt.Sprint(ours), "["+strings.TrimSpace(string(output))+"]"; got != want {
			t.Fatalf(
				"example %d (%s): gosmt=%s z3=%s\n%s",
				example, mode.name, got, want, script,
			)
		}
	}
}

func TestUnconstrainedFloatingPointAddAgreesWithPinnedZ3(t *testing.T) {
	z3 := os.Getenv("GOSMT_Z3")
	if z3 == "" {
		t.Skip("set GOSMT_Z3 to the pinned Z3 4.16.0 binary")
	}
	modes := []string{"RNE", "RNA", "RTP", "RTN", "RTZ"}
	random := rand.New(rand.NewSource(0x55464144))
	for example := 0; example < 64; example++ {
		...
		mode := modes[example%len(modes)]
		script := fmt.Sprintf(
			"(set-logic QF_FP)\n(declare-const left (_ FloatingPoint 8 24))\n(declare-const right (_ FloatingPoint 8 24))\n(assert (= (fp.to_ieee_bv (fp.add %s left right)) #x%08x))\n(check-sat)\n",
			mode, target,
		)
		ours := smtLIBExecutionStatuses(t, ExecuteSMTLib(script))
		command := exec.Command(z3, "-in", "-smt2")
...
func TestSMTLibFloatingPointSubAgreeWithPinnedZ3(t *testing.T) {
	z3 := os.Getenv("GOSMT_Z3")
	if z3 == "" {
		t.Skip("set GOSMT_Z3 to the pinned Z3 4.16.0 binary")
	}
	...
	random := rand.New(rand.NewSource(0x46505355))
	for example := 0; example < 160; example++ {
		leftPattern, rightPattern := random.Uint32(), random.Uint32()
		switch example % 32 {
		case 0:
			leftPattern, rightPattern = 0x3f800000, 0xb3800000
		case 1:
			leftPattern, rightPattern = 0x3f800000, 0x3f800000
		case 2:
			leftPattern, rightPattern = 0x7f7fffff, 0xff7fffff
		case 3:
			leftPattern, rightPattern = 0x00800000, 0x00000001
		case 4:
			leftPattern, rightPattern = 0x7f800000, 0x7f800000
		case 5:
			leftPattern, rightPattern = 0x7fc12345, 0x3f800000
		}
		mode := modes[example%len(modes)]
		...
		expression := fmt.Sprintf(
			"(fp.sub %s ((_ to_fp 8 24) #x%08x) ((_ to_fp 8 24) #x%08x))",
			mode.name, leftPattern, rightPattern,
		)
		assertion := "(fp.isNaN " + expression + ")"
		if !smt.FloatingPointIsNaN(difference) {
			bits, _ := smt.FloatingPointBits(difference).Uint64()
			assertion = fmt.Sprintf(
				"(= (fp.to_ieee_bv %s) #x%08x)", expression, uint32(bits),
			)
		}
...
func TestSMTLibSymbolicFloatingPointSubAgreeWithPinnedZ3(t *testing.T) {
	z3 := os.Getenv("GOSMT_Z3")
	if z3 == "" {
		t.Skip("set GOSMT_Z3 to the pinned Z3 4.16.0 binary")
	}
	...
	random := rand.New(rand.NewSource(0x46505353))
	for example := 0; example < 64; example++ {
		leftPattern, rightPattern := random.Uint32(), random.Uint32()
		if leftPattern&0x7f800000 == 0x7f800000 &&
			leftPattern&0x007fffff != 0 {
		...
		expected, _ := smt.FloatingPointBits(difference).Uint64()
		assertion := fmt.Sprintf(
			"(= (fp.to_ieee_bv (fp.sub %s left right)) #x%08x)",
			mode.name, uint32(expected),
		)
		if example%8 >= 4 {
			assertion = "(not " + assertion + ")"
...
func TestUnconstrainedFloatingPointSubAgreesWithPinnedZ3(t *testing.T) {
	z3 := os.Getenv("GOSMT_Z3")
	if z3 == "" {
		t.Skip("set GOSMT_Z3 to the pinned Z3 4.16.0 binary")
	}
	modes := []string{"RNE", "RNA", "RTP", "RTN", "RTZ"}
	random := rand.New(rand.NewSource(0x55465355))
	for example := 0; example < 64; example++ {
		target := random.Uint32()
		if target&0x7f800000 == 0x7f800000 &&
			target&0x007fffff != 0 {
		...
		mode := modes[example%len(modes)]
		script := fmt.Sprintf(
			"(set-logic QF_FP)\n(declare-const left (_ FloatingPoint 8 24))\n(declare-const right (_ FloatingPoint 8 24))\n(assert (= (fp.to_ieee_bv (fp.sub %s left right)) #x%08x))\n(check-sat)\n",
			mode, target,
		)
		ours := smtLIBExecutionStatuses(t, ExecuteSMTLib(script))
		command := exec.Command(z3, "-in", "-smt2")
...
func TestSMTLibFloatingPointMulAgreeWithPinnedZ3(t *testing.T) {
	z3 := os.Getenv("GOSMT_Z3")
	if z3 == "" {
		t.Skip("set GOSMT_Z3 to the pinned Z3 4.16.0 binary")
	}
	...
	for example := 0; example < 160; example++ {
		...
		switch example % 32 {
		case 0:
			leftPattern, rightPattern = 0x3fc00000, 0x40100000
		case 1:
			leftPattern, rightPattern = 0x00000001, 0x3f000000
		case 2:
			leftPattern, rightPattern = 0x7f7fffff, 0x40000000
		case 3:
			leftPattern, rightPattern = 0x00800000, 0x3f000000
		case 4:
			leftPattern, rightPattern = 0x7f800000, 0x80000000
		case 5:
			leftPattern, rightPattern = 0x7fc12345, 0x3f800000
		}
		mode := modes[example%len(modes)]
		...
		expression := fmt.Sprintf(
			"(fp.mul %s ((_ to_fp 8 24) #x%08x) ((_ to_fp 8 24) #x%08x))",
			mode.name, leftPattern, rightPattern,
		)
		assertion := "(fp.isNaN " + expression + ")"
		if !smt.FloatingPointIsNaN(product) {
			bits, _ := smt.FloatingPointBits(product).Uint64()
			assertion = fmt.Sprintf(
				"(= (fp.to_ieee_bv %s) #x%08x)", expression, uint32(bits),
			)
		}
...
func TestSMTLibSymbolicFloatingPointMulAgreeWithPinnedZ3(t *testing.T) {
	z3 := os.Getenv("GOSMT_Z3")
	if z3 == "" {
		t.Skip("set GOSMT_Z3 to the pinned Z3 4.16.0 binary")
	}
	...
	random := rand.New(rand.NewSource(0x46504d53))
	for example := 0; example < 64; example++ {
		leftPattern, rightPattern := random.Uint32(), random.Uint32()
		if leftPattern&0x7f800000 == 0x7f800000 &&
			leftPattern&0x007fffff != 0 {
		...
		expected, _ := smt.FloatingPointBits(product).Uint64()
		assertion := fmt.Sprintf(
			"(= (fp.to_ieee_bv (fp.mul %s left right)) #x%08x)",
			mode.name, uint32(expected),
		)
		if example%8 >= 4 {
			assertion = "(not " + assertion + ")"
...
func TestUnconstrainedFloatingPointMulAgreesWithPinnedZ3(t *testing.T) {
	z3 := os.Getenv("GOSMT_Z3")
	if z3 == "" {
		t.Skip("set GOSMT_Z3 to the pinned Z3 4.16.0 binary")
	}
	modes := []string{"RNE", "RNA", "RTP", "RTN", "RTZ"}
	random := rand.New(rand.NewSource(0x55464d55))
	for example := 0; example < 64; example++ {
		target := random.Uint32()
		if target&0x7f800000 == 0x7f800000 &&
			target&0x007fffff != 0 {
		...
		mode := modes[example%len(modes)]
		script := fmt.Sprintf(
			"(set-logic QF_FP)\n(declare-const left (_ FloatingPoint 8 24))\n(declare-const right (_ FloatingPoint 8 24))\n(assert (= (fp.to_ieee_bv (fp.mul %s left right)) #x%08x))\n(check-sat)\n",
			mode, target,
		)
		ours := smtLIBExecutionStatuses(t, ExecuteSMTLib(script))
		command := exec.Command(z3, "-in", "-smt2")
...
func TestSMTLibFloatingPointDivAgreeWithPinnedZ3(t *testing.T) {
	z3 := os.Getenv("GOSMT_Z3")
	if z3 == "" {
		t.Skip("set GOSMT_Z3 to the pinned Z3 4.16.0 binary")
	}
	...
	random := rand.New(rand.NewSource(0x46504456))
	for example := 0; example < 160; example++ {
		leftPattern, rightPattern := random.Uint32(), random.Uint32()
		switch example % 32 {
		case 0:
			leftPattern, rightPattern = 0x3f800000, 0x40400000
		case 1:
			leftPattern, rightPattern = 0x00000001, 0x40000000
		case 2:
			leftPattern, rightPattern = 0x7f7fffff, 0x3f000000
		case 3:
			leftPattern, rightPattern = 0x00800000, 0x40000000
		case 4:
			leftPattern, rightPattern = 0x7f800000, 0x7f800000
		case 5:
			leftPattern, rightPattern = 0x00000000, 0x80000000
		case 6:
			leftPattern, rightPattern = 0x3f800000, 0x80000000
		case 7:
			leftPattern, rightPattern = 0x7fc12345, 0x3f800000
		}
		mode := modes[example%len(modes)]
		...
		expression := fmt.Sprintf(
			"(fp.div %s ((_ to_fp 8 24) #x%08x) ((_ to_fp 8 24) #x%08x))",
			mode.name, leftPattern, rightPattern,
		)
		assertion := "(fp.isNaN " + expression + ")"
		if !smt.FloatingPointIsNaN(quotient) {
			bits, _ := smt.FloatingPointBits(quotient).Uint64()
			assertion = fmt.Sprintf(
				"(= (fp.to_ieee_bv %s) #x%08x)", expression, uint32(bits),
			)
		}
...
func TestSMTLibSymbolicFloatingPointDivAgreeWithPinnedZ3(t *testing.T) {
	z3 := os.Getenv("GOSMT_Z3")
	if z3 == "" {
		t.Skip("set GOSMT_Z3 to the pinned Z3 4.16.0 binary")
	}
	...
	random := rand.New(rand.NewSource(0x46504453))
	for example := 0; example < 64; example++ {
		leftPattern, rightPattern := random.Uint32(), random.Uint32()
		if leftPattern&0x7f800000 == 0x7f800000 &&
			leftPattern&0x007fffff != 0 {
		...
		expected, _ := smt.FloatingPointBits(quotient).Uint64()
		assertion := fmt.Sprintf(
			"(= (fp.to_ieee_bv (fp.div %s left right)) #x%08x)",
			mode.name, uint32(expected),
		)
		if example%8 >= 4 {
			assertion = "(not " + assertion + ")"
...
func TestUnconstrainedFloatingPointDivAgreesWithPinnedZ3(t *testing.T) {
	z3 := os.Getenv("GOSMT_Z3")
	if z3 == "" {
		t.Skip("set GOSMT_Z3 to the pinned Z3 4.16.0 binary")
	}
	modes := []string{"RNE", "RNA", "RTP", "RTN", "RTZ"}
	random := rand.New(rand.NewSource(0x55464456))
	for example := 0; example < 64; example++ {
		target := random.Uint32()
		if target&0x7f800000 == 0x7f800000 &&
			target&0x007fffff != 0 {
		...
		mode := modes[example%len(modes)]
		script := fmt.Sprintf(
			"(set-logic QF_FP)\n(declare-const left (_ FloatingPoint 8 24))\n(declare-const right (_ FloatingPoint 8 24))\n(assert (= (fp.to_ieee_bv (fp.div %s left right)) #x%08x))\n(check-sat)\n",
			mode, target,
		)
		ours := smtLIBExecutionStatuses(t, ExecuteSMTLib(script))
		command := exec.Command(z3, "-in", "-smt2")
...
func TestSMTLibFloatingPointFMAAgreeWithPinnedZ3(t *testing.T) {
	z3 := os.Getenv("GOSMT_Z3")
	if z3 == "" {
		t.Skip("set GOSMT_Z3 to the pinned Z3 4.16.0 binary")
	}
	...
	random := rand.New(rand.NewSource(0x4650464d))
	for example := 0; example < 160; example++ {
		leftPattern, rightPattern, addendPattern :=
			random.Uint32(), random.Uint32(), random.Uint32()
		switch example % 32 {
		case 0:
			leftPattern, rightPattern, addendPattern =
				0x3f800001, 0x3f7fffff, 0xbf800000
		case 1:
			leftPattern, rightPattern, addendPattern =
		...
		case 5:
			leftPattern, rightPattern, addendPattern =
				0x7fc12345, 0x3f800000, 0x3f800000
		}
		mode := modes[example%len(modes)]
		fused := smt.FloatingPointFMA(
			...
			smt.FloatingPointFromUint64(8, 24, uint64(leftPattern)),
			smt.FloatingPointFromUint64(8, 24, uint64(rightPattern)),
			smt.FloatingPointFromUint64(8, 24, uint64(addendPattern)),
		)
		expression := fmt.Sprintf(
			"(fp.fma %s ((_ to_fp 8 24) #x%08x) ((_ to_fp 8 24) #x%08x) ((_ to_fp 8 24) #x%08x))",
			mode.name, leftPattern, rightPattern, addendPattern,
		)
		assertion := "(fp.isNaN " + expression + ")"
		if !smt.FloatingPointIsNaN(fused) {
			bits, _ := smt.FloatingPointBits(fused).Uint64()
			assertion = fmt.Sprintf(
				"(= (fp.to_ieee_bv %s) #x%08x)", expression, uint32(bits),
			)
		}
...
func TestSMTLibSymbolicFloatingPointFMAAgreeWithPinnedZ3(t *testing.T) {
	z3 := os.Getenv("GOSMT_Z3")
	if z3 == "" {
		t.Skip("set GOSMT_Z3 to the pinned Z3 4.16.0 binary")
	}
	...
	for example := 0; example < 64; example++ {
		leftPattern, rightPattern, addendPattern :=
			random.Uint32(), random.Uint32(), random.Uint32()
		for _, pattern := range []*uint32{&leftPattern, &rightPattern, &addendPattern} {
		...
		expected, _ := smt.FloatingPointBits(fused).Uint64()
		assertion := fmt.Sprintf(
			"(= (fp.to_ieee_bv (fp.fma %s left right addend)) #x%08x)",
			mode.name, uint32(expected),
		)
		if example%8 >= 4 {
			assertion = "(not " + assertion + ")"
		...
		script := fmt.Sprintf(
			"(set-logic QF_FP)\n(declare-const left (_ FloatingPoint 8 24))\n(declare-const right (_ FloatingPoint 8 24))\n(declare-const addend (_ FloatingPoint 8 24))\n(assert (= (fp.to_ieee_bv left) #x%08x))\n(assert (= (fp.to_ieee_bv right) #x%08x))\n(assert (= (fp.to_ieee_bv addend) #x%08x))\n(assert %s)\n(check-sat)\n",
			leftPattern, rightPattern, addendPattern, assertion,
		)
		ours := smtLIBExecutionStatuses(t, ExecuteSMTLib(script))
		command := exec.Command(z3, "-in", "-smt2")
		command.Stdin = strings.NewReader(script)
...
func TestUnconstrainedFloatingPointFMAAgreesWithPinnedZ3(t *testing.T) {
	z3 := os.Getenv("GOSMT_Z3")
	if z3 == "" {
		t.Skip("set GOSMT_Z3 to the pinned Z3 4.16.0 binary")
	}
	modes := []string{"RNE", "RNA", "RTP", "RTN", "RTZ"}
	random := rand.New(rand.NewSource(0x5546464d))
	for example := 0; example < 64; example++ {
		target := random.Uint32()
		if target&0x7f800000 == 0x7f800000 &&
			target&0x007fffff != 0 {
		...
		mode := modes[example%len(modes)]
		script := fmt.Sprintf(
			"(set-logic QF_FP)\n(declare-const left (_ FloatingPoint 8 24))\n(declare-const right (_ FloatingPoint 8 24))\n(declare-const addend (_ FloatingPoint 8 24))\n(assert (= (fp.to_ieee_bv (fp.fma %s left right addend)) #x%08x))\n(check-sat)\n",
			mode, target,
		)
		ours := smtLIBExecutionStatuses(t, ExecuteSMTLib(script))
		command := exec.Command(z3, "-in", "-smt2")
...
func TestUnconstrainedFloatingPointSqrtAgreesWithPinnedZ3(t *testing.T) {
	...
	random := rand.New(rand.NewSource(0x55535152))
	for example := 0; example < 64; example++ {
		source := random.Uint32() & 0x7fffffff
		if source&0x7f800000 == 0x7f800000 &&
			source&0x007fffff != 0 {
			source = 0x3f800000
		}
		switch example % 16 {
		...
		root := smt.FloatingPointSqrt(
			mode.core,
			smt.FloatingPointFromUint64(8, 24, uint64(source)),
		)
		target, _ := smt.FloatingPointBits(root).Uint64()
		script := fmt.Sprintf(
			"(set-logic QF_FP)\n(declare-const value (_ FloatingPoint 8 24))\n(assert (= (fp.to_ieee_bv (fp.sqrt %s value)) #x%08x))\n(check-sat)\n",
			mode.name, uint32(target),
		)
		ours := smtLIBExecutionStatuses(t, ExecuteSMTLib(script))
		command := exec.Command(z3, "-in", "-smt2")
		command.Stdin = strings.NewReader(script)
		...
		if got, want := fmt.Sprint(ours), "["+strings.TrimSpace(string(output))+"]"; got != want {
			t.Fatalf(
				"example %d (%s, source=%#08x target=%#08x): gosmt=%s z3=%s\n%s",
				example, mode.name, source, uint32(target), got, want, script,
			)
		}
	}
}

func TestSMTLibFloatingPointSqrtAgreeWithPinnedZ3(t *testing.T) {
	z3 := os.Getenv("GOSMT_Z3")
	if z3 == "" {
		t.Skip("set GOSMT_Z3 to the pinned Z3 4.16.0 binary")
	}
	...
	random := rand.New(rand.NewSource(0x46505351))
	for example := 0; example < 160; example++ {
		pattern := random.Uint32()
		switch example % 32 {
		case 0:
		...
		root := smt.FloatingPointSqrt(
			mode.core,
			smt.FloatingPointFromUint64(8, 24, uint64(pattern)),
		)
		expression := fmt.Sprintf(
			"(fp.sqrt %s ((_ to_fp 8 24) #x%08x))",
			mode.name, pattern,
		)
		assertion := "(fp.isNaN " + expression + ")"
		if !smt.FloatingPointIsNaN(root) {
			bits, _ := smt.FloatingPointBits(root).Uint64()
			assertion = fmt.Sprintf(
				"(= (fp.to_ieee_bv %s) #x%08x)", expression, uint32(bits),
			)
		}
		...
		if got, want := fmt.Sprint(ours), "["+strings.TrimSpace(string(output))+"]"; got != want {
			t.Fatalf(
				"example %d (%s, %#08x): gosmt=%s z3=%s\n%s",
				example, mode.name, pattern, got, want, script,
			)
		}
	}
}

func TestSMTLibSymbolicFloatingPointSqrtAgreeWithPinnedZ3(t *testing.T) {
	z3 := os.Getenv("GOSMT_Z3")
	if z3 == "" {
		t.Skip("set GOSMT_Z3 to the pinned Z3 4.16.0 binary")
	}
	...
	random := rand.New(rand.NewSource(0x46505353))
	for example := 0; example < 64; example++ {
		pattern := random.Uint32() & 0x7fffffff
		if pattern&0x7f800000 == 0x7f800000 &&
			pattern&0x007fffff != 0 {
			pattern = 0x40000000
		}
		...
		expected, _ := smt.FloatingPointBits(root).Uint64()
		assertion := fmt.Sprintf(
			"(= (fp.to_ieee_bv (fp.sqrt %s value)) #x%08x)",
			mode.name, uint32(expected),
		)
		if example%8 >= 4 {
			assertion = "(not " + assertion + ")"
		...
		script := fmt.Sprintf(
			"(set-logic QF_FP)\n(declare-const value (_ FloatingPoint 8 24))\n(assert (= (fp.to_ieee_bv value) #x%08x))\n(assert %s)\n(check-sat)\n",
			pattern, assertion,
		)
		ours := smtLIBExecutionStatuses(t, ExecuteSMTLib(script))
		command := exec.Command(z3, "-in", "-smt2")
...
func TestSMTLibFloatingPointRemAgreeWithPinnedZ3(t *testing.T) {
	...
	random := rand.New(rand.NewSource(0x4650524d))
	for example := 0; example < 160; example++ {
		leftPattern := random.Uint32()&0x807fffff |
			uint32(100+random.Intn(51))<<23
		rightPattern := random.Uint32()&0x807fffff |
			uint32(100+random.Intn(51))<<23
		switch example % 32 {
		case 0:
			leftPattern, rightPattern = 0x40400000, 0x40000000
		case 1:
			leftPattern, rightPattern = 0x40a00000, 0x40000000
		case 2:
			leftPattern, rightPattern = 0xc0a00000, 0x40000000
		case 3:
			leftPattern, rightPattern = 0x42f60000, 0x40e00000
		case 4:
			leftPattern, rightPattern = 0x7f800000, 0x3f800000
		case 5:
			leftPattern, rightPattern = 0x3f800000, 0x00000000
		case 6:
			leftPattern, rightPattern = 0x3f800000, 0x7f800000
		case 7:
			leftPattern, rightPattern = 0x7fc12345, 0x3f800000
		}
		...
		expression := fmt.Sprintf(
			"(fp.rem ((_ to_fp 8 24) #x%08x) ((_ to_fp 8 24) #x%08x))",
			leftPattern, rightPattern,
		)
		assertion := "(fp.isNaN " + expression + ")"
		if !smt.FloatingPointIsNaN(remainder) {
			bits, _ := smt.FloatingPointBits(remainder).Uint64()
			assertion = fmt.Sprintf(
				"(= (fp.to_ieee_bv %s) #x%08x)", expression, uint32(bits),
			)
		}
		...
		if got, want := fmt.Sprint(ours), "["+strings.TrimSpace(string(output))+"]"; got != want {
			t.Fatalf(
				"example %d (%#08x rem %#08x): gosmt=%s z3=%s\n%s",
				example, leftPattern, rightPattern, got, want, script,
			)
		}
	}
}

func TestSMTLibSymbolicFloatingPointRemAgreeWithPinnedZ3(t *testing.T) {
	z3 := os.Getenv("GOSMT_Z3")
	if z3 == "" {
		t.Skip("set GOSMT_Z3 to the pinned Z3 4.16.0 binary")
	}
	...
	for example := 0; example < 64; example++ {
		...
		expected, _ := smt.FloatingPointBits(remainder).Uint64()
		assertion := fmt.Sprintf(
			"(= (fp.to_ieee_bv (fp.rem left right)) #x%08x)",
			uint32(expected),
		)
		z3Assertion := fmt.Sprintf(
			"(= (fp.to_ieee_bv (fp.rem ((_ to_fp 8 24) #x%08x) ((_ to_fp 8 24) #x%08x))) #x%08x)",
			leftPattern, rightPattern, uint32(expected),
		)
		if example%8 >= 4 {
			assertion = "(not " + assertion + ")"
...
func TestSMTLibFloatingPointToBitVectorAgreeWithPinnedZ3(t *testing.T) {
	z3 := os.Getenv("GOSMT_Z3")
	if z3 == "" {
		t.Skip("set GOSMT_Z3 to the pinned Z3 4.16.0 binary")
	}
	...
	random := rand.New(rand.NewSource(0x46504356))
	for example := 0; example < 160; example++ {
		signed := example%2 != 0
		pattern := random.Uint32()&0x807fffff |
			uint32(120+random.Intn(20))<<23
		if !signed {
			pattern &^= 0x80000000
		}
		mode := modes[example%len(modes)]
		value := smt.FloatingPointFromUint64(8, 24, uint64(pattern))
		var expected smt.BitVectorValue
		var valid bool
		operator := "fp.to_ubv"
		if signed {
			operator = "fp.to_sbv"
			expected, valid = smt.FloatingPointToSignedBitVector(
				16, mode.mode, value,
			)
		} else {
			expected, valid = smt.FloatingPointToUnsignedBitVector(
				16, mode.mode, value,
			)
		}
		if !valid {
		...
		raw, _ := expected.Uint64()
		assertion := fmt.Sprintf(
			"(= ((_ %s 16) %s ((_ to_fp 8 24) #x%08x)) #x%04x)",
			operator, mode.name, pattern, uint16(raw),
		)
		if example%16 >= 8 {
			assertion = "(not " + assertion + ")"
		}
...
func TestSMTLibSymbolicFloatingPointToBitVectorAgreeWithPinnedZ3(t *testing.T) {
	z3 := os.Getenv("GOSMT_Z3")
	if z3 == "" {
		t.Skip("set GOSMT_Z3 to the pinned Z3 4.16.0 binary")
	}
	random := rand.New(rand.NewSource(0x46504353))
	for example := 0; example < 64; example++ {
		signed := example%2 != 0
		pattern := random.Uint32()&0x807fffff |
		...
		modeName := []string{"RNE", "RNA", "RTP", "RTN", "RTZ"}[example%5]
		mode := []smt.FloatingPointRoundingMode{
		...
		if signed {
			operator = "fp.to_sbv"
			expected, valid = smt.FloatingPointToSignedBitVector(16, mode, value)
		} else {
			expected, valid = smt.FloatingPointToUnsignedBitVector(16, mode, value)
		}
		if !valid {
			t.Fatalf("test generator produced invalid example %d", example)
		}
		...
		z3Assertion := fmt.Sprintf(
			"(= ((_ %s 16) %s ((_ to_fp 8 24) #x%08x)) #x%04x)",
			operator, modeName, pattern, uint16(raw),
		)
		if example%8 >= 4 {
			assertion = "(not " + assertion + ")"
			z3Assertion = "(not " + z3Assertion + ")"
...
func TestSMTLibFloatingPointFromBitVectorAgreeWithPinnedZ3(t *testing.T) {
	...
	random := rand.New(rand.NewSource(0x42564650))
	for example := 0; example < 160; example++ {
		pattern := random.Uint32()
		signed := example%2 != 0
		mode := modes[example%len(modes)]
		value := smt.NewBitVectorUint64(32, uint64(pattern))
		operator := "to_fp_unsigned"
		var converted smt.FloatingPointValue
		if signed {
			operator = "to_fp"
			converted = smt.FloatingPointFromSignedBitVector(
				8, 24, mode.mode, value,
			)
		} else {
		...
		assertion := fmt.Sprintf(
			"(= (fp.to_ieee_bv ((_ %s 8 24) %s #x%08x)) #x%08x)",
			operator, mode.name, pattern, uint32(expected),
		)
		if example%16 >= 8 {
			assertion = "(not " + assertion + ")"
		}
...
func TestSMTLibSymbolicFloatingPointFromBitVectorAgreeWithPinnedZ3(t *testing.T) {
	z3 := os.Getenv("GOSMT_Z3")
	if z3 == "" {
		t.Skip("set GOSMT_Z3 to the pinned Z3 4.16.0 binary")
	}
	random := rand.New(rand.NewSource(0x42564653))
	for example := 0; example < 64; example++ {
		pattern := random.Uint32()
		signed := example%2 != 0
		modeNames := []string{"RNE", "RNA", "RTP", "RTN", "RTZ"}
		...
		expected, _ := smt.FloatingPointBits(converted).Uint64()
		assertion := fmt.Sprintf(
			"(= (fp.to_ieee_bv ((_ %s 8 24) %s x)) #x%08x)",
			operator, modeName, uint32(expected),
		)
		z3Assertion := fmt.Sprintf(
			"(= (fp.to_ieee_bv ((_ %s 8 24) %s #x%08x)) #x%08x)",
			operator, modeName, pattern, uint32(expected),
		)
		if example%8 >= 4 {
			assertion = "(not " + assertion + ")"
			z3Assertion = "(not " + z3Assertion + ")"
...
func TestUnconstrainedFloatingPointFromBitVectorAgreesWithPinnedZ3(
	t *testing.T,
) {
	z3 := os.Getenv("GOSMT_Z3")
	if z3 == "" {
	...
	random := rand.New(rand.NewSource(0x55425646))
	for example := 0; example < 64; example++ {
		pattern := random.Uint32()
		switch example % 16 {
		case 0:
		...
		script := fmt.Sprintf(
			"(set-logic QF_FPBV)\n(declare-const x (_ BitVec 32))\n(assert (= (fp.to_ieee_bv ((_ %s 8 24) %s x)) #x%08x))\n(check-sat)\n",
			operator, modeName, uint32(target),
		)
		ours := smtLIBExecutionStatuses(t, ExecuteSMTLib(script))
		command := exec.Command(z3, "-in", "-smt2")
		command.Stdin = strings.NewReader(script)
		...
		if got, want := fmt.Sprint(ours), "["+strings.TrimSpace(string(output))+"]"; got != want {
			t.Fatalf(
				...
				example, operator, uint32(target), got, want, script,
			)
		}
	}
}
...
func TestSMTLibFloatingPointFromRealAgreeWithPinnedZ3(t *testing.T) {
	...
	random := rand.New(rand.NewSource(0x52465047))
	for example := 0; example < 160; example++ {
		numerator := int64(random.Int31()) - 1<<30
		denominator := int64(random.Int31n(1<<20) + 1)
		numeratorTerm := fmt.Sprintf("%d.0", numerator)
		if numerator < 0 {
			numeratorTerm = fmt.Sprintf("(- %d.0)", -numerator)
		}
		modeName, mode := modeNames[example%5], modes[example%5]
		rational := smt.NewRational(numerator, denominator)
		converted := smt.FloatingPointFromRational(8, 24, mode, rational)
		expected, _ := smt.FloatingPointBits(converted).Uint64()
		assertion := fmt.Sprintf(
			"(= (fp.to_ieee_bv ((_ to_fp 8 24) %s (/ %s %d.0))) #x%08x)",
			modeName, numeratorTerm, denominator, uint32(expected),
		)
		if example%16 >= 8 {
			assertion = "(not " + assertion + ")"
		}
...
func TestSMTLibSymbolicFloatingPointFromRealAgreeWithPinnedZ3(t *testing.T) {
	z3 := os.Getenv("GOSMT_Z3")
	if z3 == "" {
		t.Skip("set GOSMT_Z3 to the pinned Z3 4.16.0 binary")
	}
	...
	random := rand.New(rand.NewSource(0x52465053))
	for example := 0; example < 64; example++ {
		numerator := int64(random.Int31()) - 1<<30
		denominator := int64(random.Int31n(1<<20) + 1)
		numeratorTerm := fmt.Sprintf("%d.0", numerator)
		...
		expected, _ := smt.FloatingPointBits(converted).Uint64()
		assertion := fmt.Sprintf(
			"(= (fp.to_ieee_bv ((_ to_fp 8 24) %s x)) #x%08x)",
			modeName, uint32(expected),
		)
		z3Assertion := fmt.Sprintf(
			"(= (fp.to_ieee_bv ((_ to_fp 8 24) %s (/ %s %d.0))) #x%08x)",
			modeName, numeratorTerm, denominator, uint32(expected),
		...
		script := fmt.Sprintf(
			"(set-logic ALL)\n(declare-const x Real)\n(assert (= x (/ %s %d.0)))\n(assert %s)\n(check-sat)\n",
			numeratorTerm, denominator, assertion,
		)
		ours := smtLIBExecutionStatuses(t, ExecuteSMTLib(script))
		z3Script := fmt.Sprintf(
			"(set-logic ALL)\n(assert %s)\n(check-sat)\n", z3Assertion,
...
func TestUnconstrainedFloatingPointToRealAgreesWithPinnedZ3(t *testing.T) {
	z3 := os.Getenv("GOSMT_Z3")
	if z3 == "" {
		t.Skip("set GOSMT_Z3 to the pinned Z3 4.16.0 binary")
	}
	random := rand.New(rand.NewSource(0x5554524c))
	for example := 0; example < 64; example++ {
		pattern := random.Uint32()
		if pattern&0x7f800000 == 0x7f800000 {
			pattern ^= 0x00800000
		...
		target := fmt.Sprintf("(/ %s %s.0)", numeratorTerm, denominator)
		script := fmt.Sprintf(
			"(set-logic ALL)\n(declare-const x (_ FloatingPoint 8 24))\n(assert (= (fp.to_real x) %s))\n(check-sat)\n",
			target,
		)
		ours := smtLIBExecutionStatuses(t, ExecuteSMTLib(script))
		command := exec.Command(z3, "-in", "-smt2")
		...
		if got, want := fmt.Sprint(ours), "["+strings.TrimSpace(string(output))+"]"; got != want {
			t.Fatalf(
				"example %d (source=%#08x): gosmt=%s z3=%s\n%s",
				example, pattern, got, want, script,
			)
		}
	}
}
...
func TestSMTLibMixedFloatingPointRealAgreeWithPinnedZ3(t *testing.T) {
	z3 := os.Getenv("GOSMT_Z3")
	if z3 == "" {
		t.Skip("set GOSMT_Z3 to the pinned Z3 4.16.0 binary")
	}
	random := rand.New(rand.NewSource(0x46504c52))
	operators := []string{"<", "<=", ">", ">="}
	for example := 0; example < 64; example++ {
		pattern := random.Uint32()
		if pattern&0x7f800000 == 0x7f800000 {
		...
		coefficient := int64(example%5 - 2)
		if coefficient == 0 {
			coefficient = 3
		}
		offset := smt.NewRational(int64(example%7-3), int64(example%5+1))
		exact := smt.AddRational(
			smt.MultiplyRational(smt.NewRational(coefficient, 1), value),
			offset,
		)
		operator := operators[example%len(operators)]
		bound := exact
		if operator == "<" {
			bound = smt.AddRational(bound, smt.NewRational(1, 5))
		} else if operator == ">" {
			bound = smt.AddRational(bound, smt.NewRational(-1, 5))
		}
		boundAssertion := fmt.Sprintf(
			"(%s r %s)", operator, smtLIBRational(bound),
		)
		if example%8 >= 4 {
			boundAssertion = "(not " + boundAssertion + ")"
		}
		bridge := fmt.Sprintf(
			"(= r (+ (* %d.0 (fp.to_real x)) %s))",
			coefficient, smtLIBRational(offset),
		)
		script := fmt.Sprintf(
			"(set-logic ALL)\n(declare-const x (_ FloatingPoint 8 24))\n(declare-const r Real)\n(assert (= (fp.to_ieee_bv x) #x%08x))\n(assert %s)\n(assert %s)\n(check-sat)\n",
			pattern, bridge, boundAssertion,
		)
		ours := smtLIBExecutionStatuses(t, ExecuteSMTLib(script))
		z3Bridge := strings.ReplaceAll(
			bridge, "(fp.to_real x)",
			fmt.Sprintf("(fp.to_real ((_ to_fp 8 24) #x%08x))", pattern),
		)
		z3Script := fmt.Sprintf(
			"(set-logic ALL)\n(declare-const r Real)\n(assert %s)\n(assert %s)\n(check-sat)\n",
			z3Bridge, boundAssertion,
		)
		command := exec.Command(z3, "-in", "-smt2")
		command.Stdin = strings.NewReader(z3Script)
		output, err := command.CombinedOutput()
