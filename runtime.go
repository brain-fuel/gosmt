...
const (
	...
	booleanFastFloatingPointEquality
	...
	booleanFastFloatingPointToReal
	...
	booleanFastIntegerProductBound
	booleanFastIntegerProductBoundSystem
...
type booleanFast struct {
	...
	floatingPointEquality         smt.FloatingPointEqualityRelation
	...
	floatingPointToReal           smt.FloatingPointToRealRelation
	...
	integerProductBound           smt.IntegerProductBound
	integerProductBoundSystem     smt.IntegerProductBoundSystem
...
func assertBoolean(assertion int, solver smt.Solver, term smt.Term[smt.BoolSort], fast booleanFast) smt.Solver {
	...
	if fast.kind == booleanFastFloatingPointEquality {
		return smt.AssertFloatingPointEqualityRelation(
			assertion, solver, fast.floatingPointEquality,
		)
	}
	...
	if fast.kind == booleanFastFloatingPointToBitVector {
		return smt.AssertFloatingPointToBitVectorRelation(
	...
	if fast.kind == booleanFastFloatingPointToReal {
		return smt.AssertFloatingPointToRealRelation(
			assertion, solver, fast.floatingPointToReal,
		)
	}
	...
	if fast.kind == booleanFastFloatingPointSub {
		return smt.AssertFloatingPointSubRelation(
			assertion, solver, fast.floatingPointSub,
		)
	}
	if fast.kind == booleanFastFloatingPointMul {
		return smt.AssertFloatingPointMulRelation(
			assertion, solver, fast.floatingPointMul,
		)
	}
	if fast.kind == booleanFastFloatingPointDiv {
		return smt.AssertFloatingPointDivRelation(
			assertion, solver, fast.floatingPointDiv,
		)
	}
	if fast.kind == booleanFastFloatingPointFMA {
		return smt.AssertFloatingPointFMARelation(
			assertion, solver, fast.floatingPointFMA,
		)
	}
	if fast.kind == booleanFastFloatingPointSqrt {
		return smt.AssertFloatingPointSqrtRelation(
...
func modelFloatingPointBits(model smt.Model, term smt.Term[smt.BitVecSort], fast bitVectorFast) (smt.BitVectorValue, bool) {
	...
	if fast.kind == bitVectorFastFloatingPointToBitVector {
		...
		value := smt.FloatingPointFromBits(
			fast.parameterA, fast.parameterB, bits,
		)
		if fast.signed {
	...
	if fast.kind == bitVectorFastFloatingPointAdd {
		...
		return smt.FloatingPointBits(smt.FloatingPointAdd(
			fast.roundingMode,
	...
	if fast.kind == bitVectorFastFloatingPointSub {
		leftBits, leftFound := smt.FloatingPointSymbolModelBits(model, fast.id)
		rightBits, rightFound := smt.FloatingPointSymbolModelBits(model, fast.firstID)
		if !leftFound || !rightFound {
			return smt.BitVectorValue{}, false
		}
		return smt.FloatingPointBits(smt.FloatingPointSub(
			fast.roundingMode,
			smt.FloatingPointFromBits(fast.parameterA, fast.parameterB, leftBits),
			smt.FloatingPointFromBits(fast.parameterA, fast.parameterB, rightBits),
		)), true
	}
	if fast.kind == bitVectorFastFloatingPointMul {
		leftBits, leftFound := smt.FloatingPointSymbolModelBits(model, fast.id)
		rightBits, rightFound := smt.FloatingPointSymbolModelBits(model, fast.firstID)
		if !leftFound || !rightFound {
			return smt.BitVectorValue{}, false
		}
		return smt.FloatingPointBits(smt.FloatingPointMul(
			fast.roundingMode,
			smt.FloatingPointFromBits(fast.parameterA, fast.parameterB, leftBits),
			smt.FloatingPointFromBits(fast.parameterA, fast.parameterB, rightBits),
		)), true
	}
	if fast.kind == bitVectorFastFloatingPointDiv {
		leftBits, leftFound := smt.FloatingPointSymbolModelBits(model, fast.id)
		rightBits, rightFound := smt.FloatingPointSymbolModelBits(model, fast.firstID)
		if !leftFound || !rightFound {
			return smt.BitVectorValue{}, false
		}
		return smt.FloatingPointBits(smt.FloatingPointDiv(
			fast.roundingMode,
			smt.FloatingPointFromBits(fast.parameterA, fast.parameterB, leftBits),
			smt.FloatingPointFromBits(fast.parameterA, fast.parameterB, rightBits),
		)), true
	}
	if fast.kind == bitVectorFastFloatingPointFMA {
		leftBits, leftFound := smt.FloatingPointSymbolModelBits(model, fast.id)
		rightBits, rightFound := smt.FloatingPointSymbolModelBits(model, fast.firstID)
		addendBits, addendFound := smt.FloatingPointSymbolModelBits(model, fast.secondID)
		if !leftFound || !rightFound || !addendFound {
			return smt.BitVectorValue{}, false
		}
		return smt.FloatingPointBits(smt.FloatingPointFMA(
			fast.roundingMode,
			smt.FloatingPointFromBits(fast.parameterA, fast.parameterB, leftBits),
			smt.FloatingPointFromBits(fast.parameterA, fast.parameterB, rightBits),
			smt.FloatingPointFromBits(fast.parameterA, fast.parameterB, addendBits),
		)), true
	}
	if fast.kind == bitVectorFastFloatingPointSqrt {
		bits, found := smt.FloatingPointSymbolModelBits(model, fast.id)
		if !found {
			return smt.BitVectorValue{}, false
		}
...
func modelFloatingPointValueBits(model Model, value FloatingPointExpr) (smt.BitVectorValue, bool) {
	if model.contextID != value.contextID {
		panic("gosmt: erased model/floating-point context mismatch")
	}
	return modelFloatingPointBits(model.core, value.bits.term, value.bits.fast)
}
...
func exactBitVectorExpr(context int, value smt.BitVectorValue) BitVecExpr {
	return bitVecExprValue{contextID: context, fast: bitVectorFast{kind: bitVectorFastValue, width: value.Width(), value: value}}
}
...
func fastEqBitVector(left, right BitVecExpr) BoolExpr {
	...
	if relation, ok := compactFloatingPointToBitVectorRelation(left.fast, right.fast); ok {
		return boolExprValue{contextID: context, fast: booleanFast{
			kind:                     booleanFastFloatingPointToBitVector,
			floatingPointToBitVector: relation,
		}}
	}
	if relation, ok := compactFloatingPointToBitVectorRelation(right.fast, left.fast); ok {
		return boolExprValue{contextID: context, fast: booleanFast{
			kind:                     booleanFastFloatingPointToBitVector,
			floatingPointToBitVector: relation,
		}}
	...
	if relation, ok := compactFloatingPointAddRelation(right.fast, left.fast); ok {
		return boolExprValue{contextID: context, fast: booleanFast{
			kind: booleanFastFloatingPointAdd, floatingPointAdd: relation,
		}}
	}
	if relation, ok := compactFloatingPointSubRelation(left.fast, right.fast); ok {
		return boolExprValue{contextID: context, fast: booleanFast{
			kind: booleanFastFloatingPointSub, floatingPointSub: relation,
		}}
	}
	if relation, ok := compactFloatingPointSubRelation(right.fast, left.fast); ok {
		return boolExprValue{contextID: context, fast: booleanFast{
			kind: booleanFastFloatingPointSub, floatingPointSub: relation,
		}}
	}
	if relation, ok := compactFloatingPointMulRelation(left.fast, right.fast); ok {
		return boolExprValue{contextID: context, fast: booleanFast{
			kind: booleanFastFloatingPointMul, floatingPointMul: relation,
		}}
	}
	if relation, ok := compactFloatingPointMulRelation(right.fast, left.fast); ok {
		return boolExprValue{contextID: context, fast: booleanFast{
			kind: booleanFastFloatingPointMul, floatingPointMul: relation,
		}}
	}
	if relation, ok := compactFloatingPointDivRelation(left.fast, right.fast); ok {
		return boolExprValue{contextID: context, fast: booleanFast{
			kind: booleanFastFloatingPointDiv, floatingPointDiv: relation,
		}}
	}
	if relation, ok := compactFloatingPointDivRelation(right.fast, left.fast); ok {
		return boolExprValue{contextID: context, fast: booleanFast{
			kind: booleanFastFloatingPointDiv, floatingPointDiv: relation,
		}}
	}
	if relation, ok := compactFloatingPointFMARelation(left.fast, right.fast); ok {
		return boolExprValue{contextID: context, fast: booleanFast{
			kind: booleanFastFloatingPointFMA, floatingPointFMA: relation,
		}}
	}
	if relation, ok := compactFloatingPointFMARelation(right.fast, left.fast); ok {
		return boolExprValue{contextID: context, fast: booleanFast{
			kind: booleanFastFloatingPointFMA, floatingPointFMA: relation,
		}}
	}
	if relation, ok := compactFloatingPointSqrtRelation(left.fast, right.fast); ok {
		return boolExprValue{contextID: context, fast: booleanFast{
			kind: booleanFastFloatingPointSqrt, floatingPointSqrt: relation,
		}}
	}
	if relation, ok := compactFloatingPointSqrtRelation(right.fast, left.fast); ok {
		return boolExprValue{contextID: context, fast: booleanFast{
			kind: booleanFastFloatingPointSqrt, floatingPointSqrt: relation,
		}}
	}
...
func compactFloatingPointToBitVectorRelation(
	expression, constant bitVectorFast,
) (smt.FloatingPointToBitVectorRelation, bool) {
	if expression.kind != bitVectorFastFloatingPointToBitVector ||
		constant.kind != bitVectorFastValue {
		return smt.FloatingPointToBitVectorRelation{}, false
	}
...
func compactFloatingPointAddRelation(
	expression, constant bitVectorFast,
) (smt.FloatingPointAddRelation, bool) {
	if expression.kind != bitVectorFastFloatingPointAdd ||
		constant.kind != bitVectorFastValue {
		return smt.FloatingPointAddRelation{}, false
	}
	return smt.NewFloatingPointAddRelation(
		expression.parameterA, expression.parameterB,
		expression.id, expression.firstID,
		expression.roundingMode, constant.value,
	), true
}

func compactFloatingPointSubRelation(
	expression, constant bitVectorFast,
) (smt.FloatingPointSubRelation, bool) {
	if expression.kind != bitVectorFastFloatingPointSub ||
		constant.kind != bitVectorFastValue {
		return smt.FloatingPointSubRelation{}, false
	}
	return smt.NewFloatingPointSubRelation(
		expression.parameterA, expression.parameterB,
		expression.id, expression.firstID,
		expression.roundingMode, constant.value,
	), true
...
func compactFloatingPointMulRelation(
	expression, constant bitVectorFast,
) (smt.FloatingPointMulRelation, bool) {
	if expression.kind != bitVectorFastFloatingPointMul ||
		constant.kind != bitVectorFastValue {
		return smt.FloatingPointMulRelation{}, false
	}
	return smt.NewFloatingPointMulRelation(
		expression.parameterA, expression.parameterB,
		expression.id, expression.firstID,
		expression.roundingMode, constant.value,
	), true
...
func compactFloatingPointDivRelation(
	expression, constant bitVectorFast,
) (smt.FloatingPointDivRelation, bool) {
	if expression.kind != bitVectorFastFloatingPointDiv ||
		constant.kind != bitVectorFastValue {
		return smt.FloatingPointDivRelation{}, false
	}
	return smt.NewFloatingPointDivRelation(
		expression.parameterA, expression.parameterB,
		expression.id, expression.firstID,
		expression.roundingMode, constant.value,
	), true
...
func compactFloatingPointFMARelation(
	expression, constant bitVectorFast,
) (smt.FloatingPointFMARelation, bool) {
	if expression.kind != bitVectorFastFloatingPointFMA ||
		constant.kind != bitVectorFastValue {
		return smt.FloatingPointFMARelation{}, false
	}
	return smt.NewFloatingPointFMARelation(
		expression.parameterA, expression.parameterB,
		expression.id, expression.firstID, expression.secondID,
		expression.roundingMode, constant.value,
	), true
}

func compactFloatingPointSqrtRelation(
	expression, constant bitVectorFast,
) (smt.FloatingPointSqrtRelation, bool) {
	if expression.kind != bitVectorFastFloatingPointSqrt ||
		constant.kind != bitVectorFastValue {
		return smt.FloatingPointSqrtRelation{}, false
	}
	...
	if product.fast.kind == integerFastProduct {
		if bound, ok := fastIntegerConstant(constant); ok {
			return boolExprValue{contextID: left.contextID, fast: booleanFast{
				...
				integerProductBound: smt.IntegerProductBound{
					LeftID:  product.fast.symbolID,
					RightID: product.fast.argumentID,
					Bound:   bound,
					Lower:   !productOnLeft,
					Strict:  strict,
				},
...
func compactBitVectorRelation(expression, constant bitVectorFast) (smt.BitVectorRelation, bool) {
	...
	case bitVectorFastAppliedSymbol:
		return smt.BitVectorRelation{Width: expression.width, SourceWidth: expression.sourceWidth, SymbolID: expression.id, Value: constant.value, Operation: expression.operation, Operand: expression.operand, ParameterA: expression.parameterA, ParameterB: expression.parameterB}, true
	}
	return smt.BitVectorRelation{}, false
}
...
func materializeBitVector(term smt.Term[smt.BitVecSort], fast bitVectorFast) smt.Term[smt.BitVecSort] {
	...
	case bitVectorFastFloatingPointToBitVector:
		value := smt.BitVecConst(
			fast.parameterA+fast.parameterB, fast.id, fast.name,
		)
		if fast.signed {
			return smt.FloatingPointToSignedBitVectorTerm(
...
func bitVectorExprContext(value BitVecExpr) int {
	return value.contextID
}

func validateFloatingPointFormat(exponentBits, significandBits int) {
	if exponentBits < 2 {
		panic("gosmt: floating-point exponent width must be at least 2")
	}
...
func floatingPointGroundValue(value FloatingPointExpr) (smt.FloatingPointValue, bool) {
	if value.bits.fast.kind != bitVectorFastValue {
		return smt.FloatingPointValue{}, false
	}
	return smt.FloatingPointFromBits(value.exponentBits, value.significandBits, value.bits.fast.value), true
}

func floatingPointParts(value FloatingPointExpr) (exponent, significand, sign BitVecExpr) {
	validateFloatingPointFormat(value.exponentBits, value.significandBits)
	total := value.exponentBits + value.significandBits
	if value.bits.fast.kind != bitVectorFastNone && value.bits.fast.width != total {
		panic("gosmt: erased floating-point bit width mismatch")
	}
	exponent = extractBitVector(total-2, value.significandBits-1, value.bits)
...
func floatingPointZero(context, width int) BitVecExpr {
	return exactBitVectorExpr(context, smt.NewBitVectorUint64(width, 0))
}

func floatingPointOnes(context, width int) BitVecExpr {
	zero := smt.NewBitVectorUint64(width, 0)
	return exactBitVectorExpr(context, smt.NotBitVectorValue(zero))
}

func floatingPointIsNaN(value FloatingPointExpr) BoolExpr {
	if ground, ok := floatingPointGroundValue(value); ok {
		return boolExprValue{contextID: value.contextID, term: smt.Bool{Value: smt.FloatingPointIsNaN(ground)}}
	}
	if compact, ok := compactFloatingPointPredicate(value, smt.FloatingPointPredicateNaN); ok {
		return compact
	}
	exponent, significand, _ := floatingPointParts(value)
	exponentAll := fastEqBitVector(exponent, floatingPointOnes(value.contextID, value.exponentBits))
	significandNonzero := fastNot(fastEqBitVector(significand, floatingPointZero(value.contextID, value.significandBits-1)))
	return fastAndPair(exponentAll, significandNonzero)
}

func floatingPointIsInfinite(value FloatingPointExpr) BoolExpr {
	if ground, ok := floatingPointGroundValue(value); ok {
		return boolExprValue{contextID: value.contextID, term: smt.Bool{Value: smt.FloatingPointIsInfinite(ground)}}
	}
	if compact, ok := compactFloatingPointPredicate(value, smt.FloatingPointPredicateInfinite); ok {
		return compact
	}
	exponent, significand, _ := floatingPointParts(value)
	return fastAndPair(
		fastEqBitVector(exponent, floatingPointOnes(value.contextID, value.exponentBits)),
		fastEqBitVector(significand, floatingPointZero(value.contextID, value.significandBits-1)),
	)
}

func floatingPointIsZero(value FloatingPointExpr) BoolExpr {
	if ground, ok := floatingPointGroundValue(value); ok {
		return boolExprValue{contextID: value.contextID, term: smt.Bool{Value: smt.FloatingPointIsZero(ground)}}
	}
	if compact, ok := compactFloatingPointPredicate(value, smt.FloatingPointPredicateZero); ok {
		return compact
	}
	exponent, significand, _ := floatingPointParts(value)
	return fastAndPair(
		fastEqBitVector(exponent, floatingPointZero(value.contextID, value.exponentBits)),
		fastEqBitVector(significand, floatingPointZero(value.contextID, value.significandBits-1)),
	)
}
...
func floatingPointIsSubnormal(value FloatingPointExpr) BoolExpr {
	if ground, ok := floatingPointGroundValue(value); ok {
		return boolExprValue{contextID: value.contextID, term: smt.Bool{Value: smt.FloatingPointIsSubnormal(ground)}}
	}
	if compact, ok := compactFloatingPointPredicate(value, smt.FloatingPointPredicateSubnormal); ok {
		return compact
	}
	exponent, significand, _ := floatingPointParts(value)
	return fastAndPair(
		fastEqBitVector(exponent, floatingPointZero(value.contextID, value.exponentBits)),
		fastNot(fastEqBitVector(significand, floatingPointZero(value.contextID, value.significandBits-1))),
	)
}

func floatingPointIsNormal(value FloatingPointExpr) BoolExpr {
	if ground, ok := floatingPointGroundValue(value); ok {
		return boolExprValue{contextID: value.contextID, term: smt.Bool{Value: smt.FloatingPointIsNormal(ground)}}
	}
	if compact, ok := compactFloatingPointPredicate(value, smt.FloatingPointPredicateNormal); ok {
		return compact
	}
	exponent, _, _ := floatingPointParts(value)
	return fastAndPair(
		fastNot(fastEqBitVector(exponent, floatingPointZero(value.contextID, value.exponentBits))),
		fastNot(fastEqBitVector(exponent, floatingPointOnes(value.contextID, value.exponentBits))),
	)
}

func floatingPointIsNegative(value FloatingPointExpr) BoolExpr {
	if ground, ok := floatingPointGroundValue(value); ok {
		return boolExprValue{contextID: value.contextID, term: smt.Bool{Value: smt.FloatingPointIsNegative(ground)}}
	}
	if compact, ok := compactFloatingPointPredicate(value, smt.FloatingPointPredicateNegative); ok {
		return compact
	}
	_, _, sign := floatingPointParts(value)
	return fastAndPair(
		fastNot(floatingPointIsNaN(value)),
		fastEqBitVector(sign, floatingPointOnes(value.contextID, 1)),
	)
}

func floatingPointIsPositive(value FloatingPointExpr) BoolExpr {
	if ground, ok := floatingPointGroundValue(value); ok {
		return boolExprValue{contextID: value.contextID, term: smt.Bool{Value: smt.FloatingPointIsPositive(ground)}}
	}
	if compact, ok := compactFloatingPointPredicate(value, smt.FloatingPointPredicatePositive); ok {
		return compact
	}
	_, _, sign := floatingPointParts(value)
	return fastAndPair(
		fastNot(floatingPointIsNaN(value)),
		fastEqBitVector(sign, floatingPointZero(value.contextID, 1)),
	)
}

func compactFloatingPointPredicate(value FloatingPointExpr, predicate uint8) (BoolExpr, bool) {
	...
	return boolExprValue{
		contextID: value.contextID,
		fast: booleanFast{
			kind: booleanFastFloatingPointRelation,
			floatingPointRelation: smt.NewFloatingPointRelation(
				value.exponentBits, value.significandBits, value.bits.fast.id, predicate,
			),
		},
	}, true
}

func floatingPointEqual(left, right FloatingPointExpr) BoolExpr {
	if left.contextID != right.contextID {
	...
	if leftOK && rightOK {
		return boolExprValue{contextID: left.contextID, term: smt.Bool{Value: smt.FloatingPointEqual(leftGround, rightGround)}}
	}
	if left.bits.fast.kind == bitVectorFastSymbol &&
		...
		return boolExprValue{
			contextID: left.contextID,
			fast: booleanFast{
				kind: booleanFastFloatingPointEquality,
				floatingPointEquality: smt.NewFloatingPointEqualityRelation(
					left.exponentBits, left.significandBits,
					left.bits.fast.id, right.bits.fast.id,
				),
			},
		}
	}
	ordinaryEqual := fastBooleanAtom(left.contextID, smt.Equal{
		Left:  materializeBitVector(left.bits.term, left.bits.fast),
		Right: materializeBitVector(right.bits.term, right.bits.fast),
	})
	zeroEqual := fastAndPair(floatingPointIsZero(left), floatingPointIsZero(right))
	equalOrZeros := fastOr([]BoolExpr{ordinaryEqual, zeroEqual})
...
func floatingPointAdd(
	...
	rightGround, rightOK := floatingPointGroundValue(right)
	if leftOK && rightOK {
		sum := smt.FloatingPointAdd(mode, leftGround, rightGround)
		return floatingPointExprValue{
			contextID: left.contextID, exponentBits: left.exponentBits,
			significandBits: left.significandBits,
			bits: exactBitVectorExpr(
				left.contextID, smt.FloatingPointBits(sum),
			),
		}
	}
	if left.bits.fast.kind != bitVectorFastSymbol ||
		right.bits.fast.kind != bitVectorFastSymbol {
		panic("gosmt: fp.add currently requires ground values or direct symbols")
	}
	total := left.exponentBits + left.significandBits
	return floatingPointExprValue{
		contextID: left.contextID, exponentBits: left.exponentBits,
		...
		bits: bitVecExprValue{
			contextID: left.contextID,
			fast: bitVectorFast{
				kind: bitVectorFastFloatingPointAdd, width: total,
				id: left.bits.fast.id, name: left.bits.fast.name,
				firstID: right.bits.fast.id, firstName: right.bits.fast.name,
				parameterA: left.exponentBits, parameterB: left.significandBits,
				roundingMode: mode,
...
func floatingPointSub(
	...
	rightGround, rightOK := floatingPointGroundValue(right)
	if leftOK && rightOK {
		difference := smt.FloatingPointSub(mode, leftGround, rightGround)
		return floatingPointExprValue{
			contextID: left.contextID, exponentBits: left.exponentBits,
			significandBits: left.significandBits,
			bits: exactBitVectorExpr(
				left.contextID, smt.FloatingPointBits(difference),
			),
		}
	}
	if left.bits.fast.kind != bitVectorFastSymbol ||
		right.bits.fast.kind != bitVectorFastSymbol {
		panic("gosmt: fp.sub currently requires ground values or direct symbols")
	}
	total := left.exponentBits + left.significandBits
	return floatingPointExprValue{
		contextID: left.contextID, exponentBits: left.exponentBits,
		...
		bits: bitVecExprValue{
			contextID: left.contextID,
			fast: bitVectorFast{
				kind: bitVectorFastFloatingPointSub, width: total,
				id: left.bits.fast.id, name: left.bits.fast.name,
				firstID: right.bits.fast.id, firstName: right.bits.fast.name,
				parameterA: left.exponentBits, parameterB: left.significandBits,
				roundingMode: mode,
...
func floatingPointMul(
	...
	rightGround, rightOK := floatingPointGroundValue(right)
	if leftOK && rightOK {
		product := smt.FloatingPointMul(mode, leftGround, rightGround)
		return floatingPointExprValue{
			contextID: left.contextID, exponentBits: left.exponentBits,
			significandBits: left.significandBits,
			bits: exactBitVectorExpr(
				left.contextID, smt.FloatingPointBits(product),
			),
		}
	}
	if left.bits.fast.kind != bitVectorFastSymbol ||
		right.bits.fast.kind != bitVectorFastSymbol {
		panic("gosmt: fp.mul currently requires ground values or direct symbols")
	}
	total := left.exponentBits + left.significandBits
	return floatingPointExprValue{
		contextID: left.contextID, exponentBits: left.exponentBits,
		...
		bits: bitVecExprValue{
			contextID: left.contextID,
			fast: bitVectorFast{
				kind: bitVectorFastFloatingPointMul, width: total,
				id: left.bits.fast.id, name: left.bits.fast.name,
				firstID: right.bits.fast.id, firstName: right.bits.fast.name,
				parameterA: left.exponentBits, parameterB: left.significandBits,
				roundingMode: mode,
...
func floatingPointDiv(
	...
	rightGround, rightOK := floatingPointGroundValue(right)
	if leftOK && rightOK {
		quotient := smt.FloatingPointDiv(mode, leftGround, rightGround)
		return floatingPointExprValue{
			contextID: left.contextID, exponentBits: left.exponentBits,
			significandBits: left.significandBits,
			bits: exactBitVectorExpr(
				left.contextID, smt.FloatingPointBits(quotient),
			),
		}
	}
	if left.bits.fast.kind != bitVectorFastSymbol ||
		right.bits.fast.kind != bitVectorFastSymbol {
		panic("gosmt: fp.div currently requires ground values or direct symbols")
	}
	total := left.exponentBits + left.significandBits
	return floatingPointExprValue{
		contextID: left.contextID, exponentBits: left.exponentBits,
		...
		bits: bitVecExprValue{
			contextID: left.contextID,
			fast: bitVectorFast{
				kind: bitVectorFastFloatingPointDiv, width: total,
				id: left.bits.fast.id, name: left.bits.fast.name,
				firstID: right.bits.fast.id, firstName: right.bits.fast.name,
				parameterA: left.exponentBits, parameterB: left.significandBits,
				roundingMode: mode,
...
func floatingPointFMA(
	...
	validateFloatingPointPair(left, right)
	validateFloatingPointPair(left, addend)
	leftGround, leftOK := floatingPointGroundValue(left)
	rightGround, rightOK := floatingPointGroundValue(right)
	addendGround, addendOK := floatingPointGroundValue(addend)
	if leftOK && rightOK && addendOK {
		fused := smt.FloatingPointFMA(
			mode, leftGround, rightGround, addendGround,
		)
		return floatingPointExprValue{
			contextID: left.contextID, exponentBits: left.exponentBits,
			significandBits: left.significandBits,
			bits: exactBitVectorExpr(
				left.contextID, smt.FloatingPointBits(fused),
			),
		}
	}
	if left.bits.fast.kind != bitVectorFastSymbol ||
		right.bits.fast.kind != bitVectorFastSymbol ||
		addend.bits.fast.kind != bitVectorFastSymbol {
		panic("gosmt: fp.fma currently requires ground values or direct symbols")
	}
	total := left.exponentBits + left.significandBits
	return floatingPointExprValue{
		contextID: left.contextID, exponentBits: left.exponentBits,
		...
		bits: bitVecExprValue{
			contextID: left.contextID,
			fast: bitVectorFast{
				kind: bitVectorFastFloatingPointFMA, width: total,
				id: left.bits.fast.id, name: left.bits.fast.name,
				firstID: right.bits.fast.id, firstName: right.bits.fast.name,
				secondID: addend.bits.fast.id, secondName: addend.bits.fast.name,
				parameterA: left.exponentBits, parameterB: left.significandBits,
				roundingMode: mode,
			},
		},
...
func floatingPointSqrt(
	...
	if ground, ok := floatingPointGroundValue(value); ok {
		root := smt.FloatingPointSqrt(mode, ground)
		return floatingPointExprValue{
			contextID: value.contextID, exponentBits: value.exponentBits,
			significandBits: value.significandBits,
			bits: exactBitVectorExpr(
				value.contextID, smt.FloatingPointBits(root),
			),
		}
	}
	if value.bits.fast.kind != bitVectorFastSymbol {
		panic("gosmt: fp.sqrt currently requires a ground value or direct symbol")
	}
	total := value.exponentBits + value.significandBits
	return floatingPointExprValue{
		contextID: value.contextID, exponentBits: value.exponentBits,
		...
		bits: bitVecExprValue{
			contextID: value.contextID,
			fast: bitVectorFast{
				kind: bitVectorFastFloatingPointSqrt, width: total,
				id: value.bits.fast.id, name: value.bits.fast.name,
				parameterA:   value.exponentBits,
				parameterB:   value.significandBits,
				roundingMode: mode,
			},
		},
	}
...
func floatingPointToBitVector(
	...
	if width <= 0 {
		panic("gosmt: floating-point conversion width must be positive")
	}
	if ground, ok := floatingPointGroundValue(value); ok {
		...
		return exactBitVectorExpr(value.contextID, converted)
	}
	if value.bits.fast.kind == bitVectorFastSymbol {
		return bitVecExprValue{
			contextID: value.contextID,
			fast: bitVectorFast{
				kind:  bitVectorFastFloatingPointToBitVector,
				width: width, id: value.bits.fast.id,
				name:         value.bits.fast.name,
				parameterA:   value.exponentBits,
				parameterB:   value.significandBits,
	...
	bits := materializeBitVector(value.bits.term, value.bits.fast)
	var term smt.Term[smt.BitVecSort]
	if signed {
		term = smt.FloatingPointToSignedBitVectorTerm(
			value.exponentBits, value.significandBits, width, bits, mode,
		)
	} else {
		term = smt.FloatingPointToUnsignedBitVectorTerm(
			value.exponentBits, value.significandBits, width, bits, mode,
		)
	}
	return bitVecExprValue{contextID: value.contextID, term: term}
...
func floatingPointToReal(value FloatingPointExpr) RealExpr {
	...
	panic("gosmt: floating-point to Real conversion currently requires a ground value or direct symbol")
}
...
func fastEqReal(left, right RealExpr) BoolExpr {
	...
	if relation, ok := compactFloatingPointToRealRelation(
...
func fastNot(value BoolExpr) BoolExpr {
	...
	if value.fast.kind == booleanFastFloatingPointEquality {
		value.fast.floatingPointEquality.Negated =
			!value.fast.floatingPointEquality.Negated
		return value
	}
	...
	if value.fast.kind == booleanFastFloatingPointToReal {
		value.fast.floatingPointToReal.Negated =
			!value.fast.floatingPointToReal.Negated
		return value
	}
	if value.fast.kind == booleanFastFloatingPointAdd {
		value.fast.floatingPointAdd.Negated = !value.fast.floatingPointAdd.Negated
		return value
	}
	if value.fast.kind == booleanFastFloatingPointSub {
		value.fast.floatingPointSub.Negated = !value.fast.floatingPointSub.Negated
		return value
	}
	if value.fast.kind == booleanFastFloatingPointMul {
		value.fast.floatingPointMul.Negated = !value.fast.floatingPointMul.Negated
		return value
	}
	if value.fast.kind == booleanFastFloatingPointDiv {
		value.fast.floatingPointDiv.Negated = !value.fast.floatingPointDiv.Negated
		return value
	}
	if value.fast.kind == booleanFastFloatingPointFMA {
		value.fast.floatingPointFMA.Negated = !value.fast.floatingPointFMA.Negated
		return value
	}
	...
	if value.fast.kind == booleanFastIntegerProductBound {
		value.fast.integerProductBound.Lower =
			!value.fast.integerProductBound.Lower
		value.fast.integerProductBound.Strict =
			!value.fast.integerProductBound.Strict
		return value
	}
...
func fastAnd(values []BoolExpr) BoolExpr {
	...
	allProductBounds := len(values) > 0
	productBoundSystem := smt.IntegerProductBoundSystem{Count: len(values)}
	if len(values) > len(productBoundSystem.Inline) {
		productBoundSystem.Overflow = make(
			[]smt.IntegerProductBound, len(values),
		)
	}
	for index, value := range values {
		if value.fast.kind != booleanFastIntegerProductBound {
			allProductBounds = false
			break
		}
		if productBoundSystem.Overflow != nil {
			productBoundSystem.Overflow[index] = value.fast.integerProductBound
		} else {
			productBoundSystem.Inline[index] = value.fast.integerProductBound
		}
	}
	if allProductBounds {
		return boolExprValue{contextID: context, fast: booleanFast{
			kind:                      booleanFastIntegerProductBoundSystem,
			integerProductBoundSystem: productBoundSystem,
		}}
	}
...
func fastAndPair(left, right BoolExpr) BoolExpr {
	...
	if left.fast.kind == booleanFastBitVectorRelation &&
		right.fast.kind == booleanFastBitVectorRelation {
		return boolExprValue{
			contextID: left.contextID,
			term: smt.BitVectorConjunction{
				Count: 2,
				Inline: [4]smt.BitVectorRelation{
					left.fast.bitVectorRelation,
					right.fast.bitVectorRelation,
				},
			},
		}
	}
...
func materializeBoolean(term smt.Term[smt.BoolSort], fast booleanFast) smt.Term[smt.BoolSort] {
	...
	case booleanFastFloatingPointEquality:
		return fast.floatingPointEquality
	...
	case booleanFastFloatingPointToReal:
		return fast.floatingPointToReal
	case booleanFastFloatingPointAdd:
		return fast.floatingPointAdd
	case booleanFastFloatingPointSub:
		return fast.floatingPointSub
	case booleanFastFloatingPointMul:
		return fast.floatingPointMul
	case booleanFastFloatingPointDiv:
		return fast.floatingPointDiv
	case booleanFastFloatingPointFMA:
	...
	case booleanFastIntegerProductBound:
		return fast.integerProductBound
	case booleanFastIntegerProductBoundSystem:
		return fast.integerProductBoundSystem
	case booleanFastUninterpretedEUFRelation:
