...
| bounded QF_NIA coupled-product model construction + three evaluations | ~10.47–10.60 us, 37,312 B, 11 allocs | ~1.297–1.431 ms, 520 B, 34 allocs | green (>122x) | green (target ≤17 allocs; 67.6% fewer) |
...
| shared unary Int→Bool predicate congruence | ~4.100–4.147 us, 12,584 B, 9 allocs | ~1.014–1.182 ms, 352 B, 23 allocs | green (>244x) | green (target ≤11 allocs) |
| shared binary Int×Int→Bool predicate congruence | ~3.955–4.005 us, 12,584 B, 9 allocs | ~0.889–1.071 ms, 400 B, 25 allocs | green (>221x) | green (target ≤12 allocs) |
...
| paired symbolic affine exact-FP-to-Real solving + model evaluation | ~13.82–13.88 us, 22,784 B, 12 allocs | ~1.346–1.709 ms, 544 B, 32 allocs | green (>96x) | green (target ≤16 allocs; 62.5% fewer) |
| mixed symbolic exact-FP-to-Real/LRA bridge + model evaluation | ~8.49–9.47 us, 26,144 B, 10 allocs | ~1.153–1.411 ms, 400 B, 25 allocs | green (>121x) | green (target ≤12 allocs; 60% fewer) |
...
| singleton-character construction | ~24.98–25.12 ns, 4 B, 1 alloc | ~500.5–736.7 ns, 48 B, 2 allocs | green (>19.9x) | green (target ≤1 alloc) |
...
| nine-symbol affine `Seq Int` relation + four-element exact models | ~37.242–37.505 us, 104,736 B, 56 allocs | ~3.236–3.311 ms, 2,888 B, 173 allocs | green (>86x) | green (target ≤86 allocs) |
| seventeen-symbol overflow affine `Seq Int` relation + exact models | ~91.043–91.578 us, 202,473 B, 117 allocs | ~5.597–5.703 ms, 5,320 B, 317 allocs | green (>61x) | green (target ≤158 allocs) |
...
The unary and binary predicate workloads make LIA establish equality between
integer arguments and then assert opposite truth values for congruent
applications. Compact Bool-valued EUF relations retain their integer argument
sorts and a canonical truth node. Unary uses 9 allocations versus Z3's 23 and
is over 244x faster; binary also uses 9 allocations versus 25 and is over 221x
faster at conservative endpoints.
...
The ground-disequality sequence workload fixes an eight-element length and
seven boundary elements, excludes the otherwise deterministic complete value,
then extracts and validates the exact model with a fresh discriminator in its
only free position. Std keeps four exclusions inline before exact overflow,
shares the 4,096-state placement contract, and backtracks fully assigned
containment placements when an excluded candidate survives ordinary positive
...
The pair-disequality sequence workload gives two roots the same seven-element
prefix, equates their lengths, and requires distinct complete values. The
length search rejects the fully fixed length-seven candidates, advances both
roots to length eight, and rebuilds the later root with the earlier model as an
exact exclusion. The same ordered exclusion step runs inside multi-relation
global candidates. It uses 21 allocations and 10.445–10.490 us versus pinned
Z3's 44 visible Go allocations and 2.603–2.701 ms. This is 52.3% fewer
allocations and over 248x conservative-endpoint throughput.

The negated symbolic-pattern sequence workload gives both roots the same
seven-element prefix, equates their lengths, and requires the second complete
root not to prefix the first. The dependency-ordered length search constructs
the nonempty pattern first, rejects the fully fixed length-seven candidates,
and advances both roots to length eight before discriminating the dependent
...
37.242–37.505 us versus pinned Z3's 173 visible Go allocations and
3.236–3.311 ms. This is 67.6% fewer allocations and over 86x
conservative-endpoint throughput.

The seventeen-symbol affine-relation workload crosses the inline boundary,
gives every root a distinct four-element prefix, fixes the total length to 68,
and extracts all seventeen exact models. Std retains the first sixteen roots
inline, grows relation normalization and candidate vectors exactly, applies
the same interval pruning, and commits the overflow models atomically. It uses
117 allocations and 91.043–91.578 us versus pinned Z3's 317 visible Go
allocations and 5.597–5.703 ms. This is 63.1% fewer allocations and over 61x
conservative-endpoint throughput.
...
The standalone first-replacement workload solves
`str.replace(x,"a","z") = "za"` and extracts the canonical model `x = "aa"`.
For nonempty source, std exhausts the unchanged target and every
replacement-aligned prefix/source/suffix preimage whose prefix has no earlier
source occurrence. Empty source has a unique prefix-removal inverse; empty
replacement enumerates code-point boundaries. Multiple constraints intersect
candidate sets under the shared 4,096-resource contract. Compact GoSMT
...
The mixed replacement/predicate workload uses the same two preimages and
requires `contains(x,"a")`, selecting `x = "a"`. Exact candidate filtering
also covers negative predicates, prefix/suffix, digit/regex membership,
length/index/conversion arithmetic, and supported Boolean forms when the
predicate owns only the replacement-anchored symbol. Foreign-symbol
dependencies remain explicit `unknown`. The cold workload uses 8 allocations
and 4.980–5.008 us versus pinned Z3's 18 allocations and 1.329–1.410 ms. This
is 55.6% fewer allocations and over 265x conservative-endpoint throughput.
...
The floating-point addition workload fixes binary32 symbols to `1.5` and
`2.25`, adds them under round-nearest-ties-to-even, constrains the exact `3.75`
IEEE result, and evaluates the derived sum. SMT-LIB streaming execution uses
11 allocations and 3.922–3.958 us versus the complete parser's 185
allocations and 11.680–11.809 us: 94.1% fewer allocations and over 2.95x
conservative-endpoint throughput. The separately gated typed workload uses
10 allocations and 8.107–8.130 us versus pinned Z3's 21 visible Go
...
uses 11 allocations and 3.958–4.090 us versus the complete parser's 185
allocations and 11.348–11.577 us: 94.1% fewer allocations and over 2.77x
conservative-endpoint throughput. The separately gated typed workload uses
10 allocations and 8.507–8.723 us versus pinned Z3's 21 visible Go
allocations and 1.292–1.386 ms: 52.4% fewer allocations and over 148x
conservative-endpoint throughput.

The unconstrained floating-point subtraction workload fixes only the exact
binary32 result to `1.5`, synthesizes two operand models, and evaluates both.
GoSMT's validated result-minus-signed-zero path uses 5 allocations and
4.444–4.506 us versus pinned Z3's 17 visible Go allocations and
4.838–5.525 ms: 70.6% fewer allocations and over 1,073x
conservative-endpoint throughput. The paired SMT-LIB workload uses 9
allocations and 4.600–5.238 us versus the complete parser's 200 allocations
and 14.154–16.933 us: 95.5% fewer allocations and over 2.70x
conservative-endpoint throughput.

The floating-point multiplication workload fixes binary32 symbols to `1.5`
and `2.25`, multiplies under round-nearest-ties-to-even, constrains the exact
`3.375` IEEE result, and evaluates the derived product. SMT-LIB streaming
execution uses 11 allocations and 4.007–4.075 us versus the complete parser's
185 allocations and 11.918–11.980 us: 94.1% fewer allocations and over 2.92x
conservative-endpoint throughput. The separately gated typed workload uses
10 allocations and 8.607–8.717 us versus pinned Z3's 21 visible Go
...
The unconstrained floating-point division workload fixes only the exact
binary32 result to the nearest representation of `1/3`, synthesizes two
operand models, and evaluates both. GoSMT's validated result-divided-by-one
path uses 5 allocations and 4.590–4.606 us versus pinned Z3's 17 visible Go
allocations and 250.326–312.880 ms: 70.6% fewer allocations and over 54,000x
conservative-endpoint throughput. The paired SMT-LIB workload uses 9
allocations and 4.822–4.893 us versus the complete parser's 200 allocations
...
validates the synthesized binary32 source. GoSMT uses 5 allocations and
4.470–4.974 us versus pinned Z3's 13 allocations and 34.088–45.590 ms: 61.5%
fewer allocations and over 6,850x conservative-endpoint throughput. The
combined SMT-LIB repeated-subtraction/division/remainder workload uses 11
allocations and 5.434–6.726 us versus the complete parser's 218 allocations
...
uses 13 allocations and 4.809–5.501 us versus the complete parser's 236
allocations and 15.973–19.560 us: 94.5% fewer allocations and over 2.90x
conservative-endpoint throughput. The separately gated typed workload uses
11 allocations and 11.273–11.332 us versus pinned Z3's 26 visible Go
...
versus the complete parser's 138 allocations and 9.233–11.652 us: 93.5% fewer
allocations and over 2.18x conservative-endpoint throughput. The separately
gated typed workload uses 9 allocations and 7.714–8.614 us versus pinned Z3's
...
The floating-point-to-bit-vector workload covers signed and unsigned
binary32 conversion under round-nearest-ties-to-even, constrains exact
eight-bit results, and evaluates both derived values. SMT-LIB streaming
execution uses 9 allocations and 3.392–3.456 us versus the complete parser's
140 allocations and 8.378–10.311 us: 93.6% fewer allocations and over 2.42x
conservative-endpoint throughput. The separately gated typed workload uses
...
Z3's 14 visible Go allocations and 3.208–3.808 ms: 64.3% fewer allocations
and over 666x conservative-endpoint throughput. The four-image SMT-LIB
workload spans binary16, binary32, binary64, and binary128, using 140
allocations and 8.062–9.776 us versus the complete parser's 420 allocations
...
The unconstrained Real-to-floating-point workload fixes only the binary32
result to `-3` and synthesizes an exact rational source. GoSMT's
target-derived, forward-validated path uses 6 allocations and 3.314–3.567 us
versus pinned Z3's 13 visible Go allocations and 4.911–5.117 ms: 53.8% fewer
allocations and over 1,377x conservative-endpoint throughput. The four-image
binary32 SMT-LIB workload uses 29 allocations and 5.172–6.287 us versus the
...
uses 56 allocations and 7.422–8.734 us versus the complete parser's 365
allocations and 23.667–23.843 us: 84.7% fewer allocations and over 2.70x
conservative-endpoint throughput. The separately gated typed workload uses 12
allocations and 13.822–13.877 us versus pinned Z3's 32 visible Go allocations
and 1.346–1.709 ms: 62.5% fewer allocations and over 96x
conservative-endpoint throughput.

The mixed floating-point/linear-real workload fixes a binary32 symbol to
`1.5`, equates its exact Real conversion with an otherwise unconstrained Real
symbol, applies an independent strict LRA bound, and evaluates both the Real
symbol and cross-theory difference. SMT-LIB streaming execution uses 82
allocations and 9.708–11.575 us versus the complete parser's 348 allocations
and 25.450–30.731 us: 76.4% fewer allocations and over 2.19x
conservative-endpoint throughput. The separately gated typed workload uses 10
allocations and 8.493–9.470 us versus pinned Z3's 25 visible Go allocations
and 1.153–1.411 ms: 60% fewer allocations and over 121x
conservative-endpoint throughput.
