...
func Assert(assertion int, solver Solver, formula BoolExpr) Solver {
	...
	case solverValue:
		...
		switch __gp_m60 := any(formula).(type) {
		case boolExprValue:
			formulaContext := __gp_m60.contextID
			term := __gp_m60.term
			fast := __gp_m60.fast
...
func Check(solver Solver) Result {
	switch __gp_m61 := any(solver).(type) {
	case solverValue:
