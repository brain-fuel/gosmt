...
| Algebraic datatypes | foundation: Go+ declaration-identity/cardinality-indexed sorts; nullary, arbitrary-arity same-sort recursive, scalar product, mixed-sort recursive, and mutually recursive constructor groups; type-level field-signature and argument HLists; target-indexed cross-declaration references; sort-refining selector cursors; Bool/Int/Real/bit-vector/Self/reference fields; productivity validation; recognizers; equality/disequality; constructor disjointness and per-field injectivity; graph acyclicity across declaration boundaries; finite-domain coloring; exact mixed and mutual model values including recognizer-only fields; compact inline storage through arity four; context-indexed GoSMT façade; and SMT-LIB `declare-datatype`/`declare-datatypes` execution | 64 finite-enumeration, 64 unary-depth, 64 binary-branching, and 64 ternary-branching systems plus mixed/mutual scalar, injectivity, acyclicity, productivity, recognizer-only, SMT-LIB, and pinned-Z3 cases; cross-datatype, wrong-target, wrong-arity, wrong-field-order, wrong-sort, and wrong-context rejection; finite, unary-, binary-, n-ary-, mixed-, and mutually-recursive official-API cold model gates |
| General recursive datatypes and sequences/strings | planned: parametric datatypes, match/update, complete Boolean QF_DT, sequences and strings | remaining QF_DT, QF_S, QF_SLIA |
