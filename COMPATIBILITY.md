...
| Strings, sequences, and regular languages | foundation: context-indexed GoSMT strings and regular expressions; compact inline string and postfix-regex construction with constant folding and direct fixed-bitset evaluation; SMT-LIB `String` literals/constants and the full 0..0x2ffff character domain through Unicode/WTF-8 escapes; `str.++`, `str.len`, contains/prefix/suffix, indexed at/substr/indexof, first/all replacement, exact arbitrary-precision `str.to_int`/`str.from_int`, code-point conversion, ASCII digit recognition, equality/disequality, exact model values, regex membership, literal/range/empty/full/all-character languages, concat/union/intersection/difference/complement, star/plus/optional, and exact/bounded indexed repetition; constructive shortest and shared-witness models over conjunctive positive/negative regular constraints; equality-forced and empty-singleton-intersection contradiction proofs; Go+ element-sort-indexed generic sequence and regex constructors | separate 64-case deterministic expanded QF_SLIA, ground QF_S, symbolic QF_S, and interacting QF_S systems agree with pinned Z3; ground, constructed-symbol, shared-symbol, and fixed-symbol Go+/SMT-LIB model laws including invalid ranges, empty languages, Unicode, surrogate, arbitrary-precision, and out-of-range behavior; base, indexed, conversion, ground-regex, symbolic-regex, and interacting-regex official-API cold model throughput/allocation gates |
| General recursive datatypes and remaining sequence/string operations | in progress | remaining QF_DT combinations; complete non-conjunctive symbolic regular constraints, general sequence evaluation, and general word equations |
