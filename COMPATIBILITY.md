...
| General linear integer arithmetic | foundation: exact arbitrary-precision affine normalization, constant multiplication, integer strictness, rational-simplex relaxation, branch-and-bound integrality, finite resource limits, exact models, and SMT-LIB `<`/`<=`/`>`/`>=` | 96 deterministic bounded QF_LIA systems plus explicit divisibility/model laws agree with pinned Z3; official-API cold model gate |
