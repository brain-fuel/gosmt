...
| General linear integer arithmetic | foundation: exact arbitrary-precision affine normalization, constant multiplication, integer strictness, rational-simplex relaxation, branch-and-bound integrality, finite resource limits, exact models, compact coefficient/problem arenas, and SMT-LIB `<`/`<=`/`>`/`>=` | 96 deterministic bounded QF_LIA systems plus explicit divisibility/model laws agree with pinned Z3; compact-equation and general multi-row official-API cold model gates |
...
The QF_LIA foundation decides conjunctive affine integer systems with exact
coefficients and models. A 4,096-node branch limit returns `unknown` explicitly
rather than approximating difficult integer programs. Boolean structure around
LIA atoms, cutting planes, divisibility propagation beyond the compact
single-equation path, and complete LIA-to-array/EUF equality exchange remain
planned.
