...
| Arrays | foundation: sort-indexed `Array[I,E]`, symbols, constant arrays, `select`, `store`, sound ground read-over-write normalization, equality-driven select congruence, symbolic integer- and bit-vector-index equivalence, finite integer-array models, extensional witnesses, store-chain extensionality, exception-aware cross-base equality, exact symbolic-to-constant defaults, disjoint array/IDL/BV products, and IDL/BV-implied shared-index equality exchange; width-indexed GoSMT bit-vector-array façade; SMT-LIB `(Array Int Int)` and QF_AUFBV execution | cross-package index/element sort rejection; direct and nested store/select laws; 704 deterministic array, mixed QF_AUFLIA, and QF_AUFBV cases agree with pinned Z3; ten official-API cold performance gates |
