...
| Algebraic datatypes | foundation: Go+ declaration-identity/cardinality-indexed sorts; nullary, arbitrary-arity same-sort recursive, scalar product, and mixed-sort recursive constructors; type-level field-signature and argument HLists; sort-refining selector cursors; Bool/Int/Real/bit-vector/Self fields; recognizers; equality/disequality; constructor disjointness and per-field injectivity; graph acyclicity; finite-domain coloring; exact mixed model values including recognizer-only synthetic fields; compact inline storage through arity four; context-indexed GoSMT façade; and SMT-LIB `declare-datatype` execution | 64 finite-enumeration, 64 unary-depth, 64 binary-branching, and 64 ternary-branching systems plus mixed scalar, injectivity, acyclicity, recognizer-only, SMT-LIB, and pinned-Z3 cases; cross-datatype, wrong-arity, wrong-field-order, wrong-sort, and wrong-context rejection; finite, unary-, binary-, n-ary-, and mixed-recursive official-API cold model gates |
| General recursive datatypes and sequences/strings | planned: mutually recursive declarations, parametric datatypes, match/update, complete Boolean QF_DT, sequences and strings | remaining QF_DT, QF_S, QF_SLIA |
