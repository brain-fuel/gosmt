...
| Finite enumeration datatypes | foundation: Go+ declaration-identity/cardinality-indexed sorts, nullary constructors, recognizers, equality/disequality, finite-domain coloring, exhaustion detection, exact model values, and SMT-LIB `declare-datatype` execution | 64 deterministic constructor/recognizer systems agree with pinned Z3; cross-datatype compile rejection; official-API cold model gate |
| Recursive datatypes and sequences/strings | planned | recursive QF_DT, QF_S, QF_SLIA |
