...
| Strings and sequences | foundation: context-indexed GoSMT strings; compact inline string-relation systems; SMT-LIB `String` literals/constants, `str.++`, `str.len`, `str.contains`, `str.prefixof`, `str.suffixof`, equality/disequality, Unicode code-point lengths, and exact model values; Go+ element-sort-indexed generic sequence empty/unit/concat/length constructors | 64 deterministic QF_SLIA systems agree with pinned Z3; ground and symbolic Go+/SMT-LIB model laws; official-API cold model throughput and allocation gates |
| General recursive datatypes and remaining sequence/string operations | in progress | remaining QF_DT combinations; sequence extract/at/index/replace, regexes, and general word equations |
