...
| Strings and sequences | foundation: context-indexed GoSMT strings; compact inline string-relation systems and constant folding; SMT-LIB `String` literals/constants and the full 0..0x2ffff character domain through Unicode/WTF-8 escapes; `str.++`, `str.len`, contains/prefix/suffix, indexed at/substr/indexof, first/all replacement, exact arbitrary-precision `str.to_int`/`str.from_int`, code-point conversion, ASCII digit recognition, equality/disequality, and exact model values; Go+ element-sort-indexed generic sequence empty/unit/concat/length constructors | 64 deterministic expanded QF_SLIA systems agree with pinned Z3; ground and symbolic Go+/SMT-LIB model laws including invalid, empty, negative, surrogate, arbitrary-precision, and out-of-range behavior; base, indexed, and conversion official-API cold model throughput/allocation gates |
| General recursive datatypes and remaining sequence/string operations | in progress | remaining QF_DT combinations; general sequence evaluation, regexes, and general word equations |
