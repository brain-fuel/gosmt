...
| Strings and sequences | foundation: context-indexed GoSMT strings; compact inline string-relation systems and constant folding; SMT-LIB `String` literals/constants and Unicode escapes; `str.++`, `str.len`, `str.contains`, `str.prefixof`, `str.suffixof`, `str.at`, `str.substr`, `str.indexof`, first-occurrence `str.replace`, equality/disequality, Unicode code-point indexing, and exact model values; Go+ element-sort-indexed generic sequence empty/unit/concat/length constructors | 64 deterministic expanded QF_SLIA systems agree with pinned Z3; ground and symbolic Go+/SMT-LIB model laws including empty/out-of-range behavior; base and indexed official-API cold model throughput/allocation gates |
| General recursive datatypes and remaining sequence/string operations | in progress | remaining QF_DT combinations; general sequence evaluation, replace-all, conversions, regexes, and general word equations |
